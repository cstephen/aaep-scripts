...
var drupalInitPromise = drupalAgent.initialize({
  ...
  for(var id in results) { 
    var geoJson = JSON.parse(results[id].geofield);
    var point;
    ...
    var themeArray = results[id].theme.split(':');
    for(var i = 0; i < themeArray.length; i++) {
      items.push({
        id: results[id].nid,
        title: results[id].node_title,
        snippet: results[id].description,
        link: drupalBaseUrl + results[id].link,
        url: results[id].url,
        ...
        ordinal: i,
        theme: themeArray[i],
        image: drupalBaseUrl + results[id].image
      })
    }
  }
